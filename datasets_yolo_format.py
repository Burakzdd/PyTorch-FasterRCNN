...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        boxes = []
        labels = []     
        file_names = [(image_path.split("/")[-1]).split(".")[0] for image_path in self.image_paths]
        image = cv2.imread(self.dir_path+file_names[idx]+".jpg")
        image_height,image_width,_ = image.shape
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB).astype(np.float32)
        image_resized = cv2.resize(image, (self.width, self.height))
        image_resized /= 255.0
        with open(self.dir_path+file_names[idx]+".txt") as f:
            for line in f:
                bbox_coordinates = line.split(" ")
                labels.append(self.classes[int(bbox_coordinates[0])])
                ...
                xmax_final = (x_min/image_width) * self.width
                ymin_final = (x_min/image_height) * self.height
                yamx_final = (x_min/image_height) * self.height
